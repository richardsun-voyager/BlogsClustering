...
#Read the links
fh = open('blogs.txt')#Open the file of blog links
log_error = open('DSC/log_error.txt','w+')#Record the inaccessible links
popularity = open('DSC/popularity.txt','w+')#Create a file to record comments, like count and facebook count of each blog
for line in fh:
    try:
        ...
        if(len(blog)>0):
            ...
            t = open('DSC/'+link.lstrip('http://www.datasciencecentral.com/profiles/blogs/') + '.txt','w')
