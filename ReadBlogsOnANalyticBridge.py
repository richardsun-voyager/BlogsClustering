#Read all the blogs on Data Science Central Website
import urllib2
import re
import os
import cookielib
...
fh = open('blogs2.txt')#Open the file of blog links
log_error = open('AnalyticBridge/log_error.txt','w+')#Record the inaccessible links
popularity = open('AnalyticBridge/popularity.txt','w+')#Create a file to record comments, like count and facebook count of each blog
for line in fh:
    try:
        ...
        if(len(blog)>0):
            ...
            t = open('AnalyticBridge/'+link.lstrip('http://www.analyticbridge.com/profiles/blogs/') + '.txt','w')
